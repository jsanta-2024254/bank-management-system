...
export const getFavorites = async (req, res) => {
    try {
        const favorites = await Favorite.find({ usuario: req.user.id })
            .populate('cuenta', 'numeroCuenta tipoCuenta saldo estado')
            .sort({ createdAt: -1 });
...
export const transferToFavorite = async (req, res) => {
    ...
    try {
        ...
        const cuentaOrigen = await Account.findOne({ usuario: req.user.id, estado: true }).session(session);

        if (!cuentaOrigen || cuentaOrigen.saldo < montoNum) {
            await session.abortTransaction();
            return res.status(400).json({ success: false, message: 'Saldo insuficiente o cuenta inactiva' });
        }

        // 2. Verificar límite diario (Lógica de negocio)
        const hoy = new Date().toISOString().split('T')[0];
        let dailyLimit = await DailyLimit.findOne({ usuario: req.user.id, fecha: hoy }).session(session);
        ...
        if (totalHoy + montoNum > 10000) {
            ...
            return res.status(400).json({ 
                success: false, 
                message: `Límite diario excedido. Disponible: Q${(10000 - totalHoy).toFixed(2)}` 
            });
        }
        ...
        if (dailyLimit) {
            ...
            await DailyLimit.create([{ usuario: req.user.id, fecha: hoy, totalTransferido: montoNum }], { session });
        }

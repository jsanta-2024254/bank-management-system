...
// ── 2FA: Enviar código al correo ───────────────────────────
export const sendTwoFactorHelper = async (emailOrUsername) => {
  try {
    const user = await findUserByEmailOrUsername(emailOrUsername);
    ...
    // Guardar o reemplazar código en la BD
    // destroy + create para evitar conflictos de upsert en PostgreSQL
    await UserTwoFactor.destroy({ where: { UserId: user.Id } });
    ...
    return {
      success: true,
      message: 'Código 2FA enviado al correo. Tienes 5 minutos para usarlo.',
    };
  } catch (error) {
    console.error('Error en sendTwoFactorHelper:', error);
    throw error;
  }
};

// ── 2FA: Verificar código y devolver JWT ──────────────────
export const verifyTwoFactorHelper = async (emailOrUsername, code) => {
  try {
    const user = await findUserByEmailOrUsername(emailOrUsername);
    ...
    const twoFactorRecord = user.UserTwoFactor;

    if (!twoFactorRecord) {
      throw new Error('No hay un código 2FA activo. Solicita uno nuevo.');
    }

    if (twoFactorRecord.IsUsed) {
      throw new Error('Este código ya fue utilizado. Solicita uno nuevo.');
    }

    if (new Date() > new Date(twoFactorRecord.TwoFactorCodeExpiry)) {
      throw new Error('El código 2FA ha expirado. Solicita uno nuevo.');
    }
    ...
    await twoFactorRecord.update({ IsUsed: true });

    // Generar JWT usando tu función existente (igual que en loginUserHelper)
    const role = user.UserRoles?.[0]?.Role?.Name || 'USER_ROLE';
    const token = await generateJWT(user.Id.toString(), { role });
    ...
    console.error('Error en verifyTwoFactorHelper:', error);
    throw error;
  }
};
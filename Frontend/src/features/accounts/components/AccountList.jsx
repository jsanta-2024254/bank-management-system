import { useEffect, useState } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { Banknote, Edit3, Trash2, Plus, CreditCard, Search, WalletCards } from 'lucide-react'
import { toast } from 'react-hot-toast'
import useAccountStore from '../store/accountStore'
import useAuthStore from '../../auth/store/authStore'
import AccountForm from './AccountForm'
import DepositForm from '../../deposits/components/DepositForm'
import ConfirmDialog from '../../../shared/components/ui/ConfirmDialog'

const getUserRole = (user) => {
    return (
        user?.role ||
        user?.Role ||
        user?.roles?.[0] ||
        user?.Roles?.[0] ||
        'USER_ROLE'
    )
}

const getUsuarioTexto = (usuario) => {
    if (!usuario) return ''
    if (typeof usuario === 'string') return usuario
    return usuario.username || usuario.email || usuario.name || usuario._id || usuario.id || ''
}

const AccountList = () => {
    const user = useAuthStore((state) => state.user)
    const role = getUserRole(user)
    const esAdmin = role === 'ADMIN_ROLE'

    const { accounts, loading, error, fetchAccounts, deleteAccount } = useAccountStore()
    const [showForm, setShowForm] = useState(false)
    const [editingAccount, setEditingAccount] = useState(null)
    const [confirmId, setConfirmId] = useState(null)
    const [depositAccount, setDepositAccount] = useState(null)
    const [search, setSearch] = useState('')

    useEffect(() => {
        fetchAccounts()
    }, [fetchAccounts])

    const handleDelete = async () => {
        const toastId = toast.loading('Eliminando cuenta...')

        try {
            await deleteAccount(confirmId)
            setConfirmId(null)
            toast.success('Cuenta eliminada correctamente', { id: toastId })
        } catch (error) {
            toast.error(
                error?.response?.data?.message || 'Error al eliminar la cuenta',
                { id: toastId }
            )
        }
    }

    const filtered = accounts.filter((account) => {
        const query = search.toLowerCase()
        const usuarioTexto = getUsuarioTexto(account.usuario).toLowerCase()

        return (
            String(account.numeroCuenta || '').toLowerCase().includes(query) ||
            String(account.tipoCuenta || '').toLowerCase().includes(query) ||
            (esAdmin && usuarioTexto.includes(query))
        )
    })

    const fmt = (n) =>
        new Intl.NumberFormat('es-GT', {
            style: 'currency',
            currency: 'GTQ',
        }).format(n || 0)

    const totalColumns = esAdmin ? 6 : 3
    const cuentasActivas = accounts.filter((account) => account.estado).length
    const saldoTotal = accounts.reduce((sum, account) => sum + Number(account.saldo || 0), 0)

    return (
        <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="pb-10"
        >
            {showForm && (
                <AccountForm
                    account={editingAccount}
                    onClose={() => {
                        setShowForm(false)
                        setEditingAccount(null)
                        fetchAccounts()
                    }}
                />
            )}

            {depositAccount && (
                <DepositForm
                    cuentaPreseleccionada={depositAccount}
                    onClose={() => {
                        setDepositAccount(null)
                        fetchAccounts()
                    }}
                />
            )}

            {confirmId && (
                <ConfirmDialog
                    message="Esta acción desactivará la cuenta bancaria."
                    onConfirm={handleDelete}
                    onCancel={() => setConfirmId(null)}
                />
            )}

            <div className="mb-8 overflow-hidden rounded-4xl border border-[#d7bc73]/45 bg-[#fffaf0]/62 px-6 py-6 shadow-[0_22px_60px_rgba(92,64,19,0.1)] backdrop-blur-xl md:px-8">
                <div className="premium-gold-line mb-6 h-px w-full" />

                <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex items-center gap-4">
                        <div className="flex h-16 w-16 items-center justify-center rounded-3xl border border-[#c89b3c]/50 bg-linear-to-br from-[#fff8df] via-[#ead190] to-[#9a6b16] shadow-[0_18px_38px_rgba(154,107,22,0.24)]">
                            <WalletCards size={28} className="text-[#5b3a0d]" />
                        </div>

                        <div>
                            <p className="mb-1 text-[10px] font-black uppercase tracking-[0.28em] text-[#9a6b16]/75">
                                Gestión bancaria
                            </p>

                            <h1 className="text-3xl font-black tracking-tight text-[#3f2c12] md:text-4xl">
                                Cuentas Bancarias
                            </h1>

                            <p className="mt-1 text-sm font-semibold text-[#7a6849]">
                                {esAdmin
                                    ? 'Gestiona las cuentas monetarias y de ahorro.'
                                    : 'Consulta tus cuentas monetarias y de ahorro.'}
                            </p>
                        </div>
                    </div>

                    <button
                        onClick={() => {
                            setEditingAccount(null)
                            setShowForm(true)
                        }}
                        className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[#c89b3c]/55 bg-linear-to-r from-[#b98219] via-[#d9b45e] to-[#8a611b] px-6 py-3.5 text-sm font-black text-white shadow-[0_18px_36px_rgba(154,107,22,0.25)] transition-all hover:-translate-y-0.5 hover:shadow-[0_22px_44px_rgba(154,107,22,0.32)] active:scale-95 sm:w-auto"
                    >
                        <Plus size={18} />
                        Nueva Cuenta
                    </button>
                </div>

                <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div className="rounded-3xl border border-[#d7bc73]/40 bg-white/42 p-5">
                        <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                            Total de cuentas
                        </p>
                        <p className="mt-2 text-2xl font-black text-[#3f2c12]">
                            {accounts.length}
                        </p>
                    </div>

                    <div className="rounded-3xl border border-[#d7bc73]/40 bg-white/42 p-5">
                        <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                            Cuentas activas
                        </p>
                        <p className="mt-2 text-2xl font-black text-[#3f2c12]">
                            {cuentasActivas}
                        </p>
                    </div>

                    <div className="rounded-3xl border border-[#d7bc73]/40 bg-white/42 p-5">
                        <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                            Saldo consolidado
                        </p>
                        <p className="mt-2 text-2xl font-black text-[#3f2c12]">
                            {fmt(saldoTotal)}
                        </p>
                    </div>
                </div>
            </div>

            <div className="relative mb-5">
                <Search
                    size={16}
                    className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9a6b16]/70"
                />

                <input
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    placeholder={
                        esAdmin
                            ? 'Buscar por número, tipo o usuario...'
                            : 'Buscar por número o tipo...'
                    }
                    className="w-full rounded-2xl border border-[#d7bc73]/50 bg-white/58 py-3.5 pl-10 pr-4 text-sm font-semibold text-[#3b2a14] placeholder-[#a89365] shadow-[inset_0_1px_0_rgba(255,255,255,0.55)] transition-all focus:border-[#b98219]/70 focus:bg-white/80 focus:outline-none focus:ring-4 focus:ring-[#d9b45e]/18"
                />
            </div>

            {error && (
                <div className="mb-6 rounded-2xl border border-red-200 bg-red-50/80 px-4 py-3 text-center text-sm font-semibold text-red-700">
                    {error}
                </div>
            )}

            <div className="relative overflow-hidden rounded-4xl border border-[#d7bc73]/45 bg-[#fffaf0]/68 shadow-[0_22px_60px_rgba(92,64,19,0.1)] backdrop-blur-xl">
                <div className="premium-gold-line absolute left-8 right-8 top-0 h-px" />

                <div className="custom-scrollbar overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-[#d7bc73]/28 bg-[#ead9ad]/22">
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                                    Cuenta
                                </th>
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                                    Tipo
                                </th>
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                                    Saldo
                                </th>
                                {esAdmin && (
                                    <>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                                            Usuario
                                        </th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                                            Estado
                                        </th>
                                        <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-[0.24em] text-[#8a611b]/70">
                                            Acciones
                                        </th>
                                    </>
                                )}
                            </tr>
                        </thead>

                        <tbody className="divide-y divide-[#d7bc73]/28">
                            {loading && accounts.length === 0 ? (
                                [1, 2, 3].map((i) => (
                                    <tr key={i}>
                                        <td className="px-8 py-5">
                                            <div className="h-10 w-44 animate-pulse rounded-xl bg-[#ead9ad]/70" />
                                        </td>
                                        <td className="px-8 py-5">
                                            <div className="h-6 w-24 animate-pulse rounded-full bg-[#ead9ad]/70" />
                                        </td>
                                        <td className="px-8 py-5">
                                            <div className="h-5 w-28 animate-pulse rounded-lg bg-[#ead9ad]/70" />
                                        </td>
                                        {esAdmin && (
                                            <>
                                                <td className="px-8 py-5">
                                                    <div className="h-5 w-32 animate-pulse rounded-lg bg-[#ead9ad]/70" />
                                                </td>
                                                <td className="px-8 py-5">
                                                    <div className="h-6 w-16 animate-pulse rounded-full bg-[#ead9ad]/70" />
                                                </td>
                                                <td className="px-8 py-5">
                                                    <div className="ml-auto h-8 w-20 animate-pulse rounded-lg bg-[#ead9ad]/70" />
                                                </td>
                                            </>
                                        )}
                                    </tr>
                                ))
                            ) : filtered.length === 0 ? (
                                <tr>
                                    <td
                                        colSpan={totalColumns}
                                        className="px-8 py-16 text-center text-[#8a6a3a]"
                                    >
                                        <CreditCard size={34} className="mx-auto mb-3 opacity-40" />
                                        <p className="font-bold">No se encontraron cuentas</p>
                                    </td>
                                </tr>
                            ) : (
                                <AnimatePresence>
                                    {filtered.map((account) => (
                                        <motion.tr
                                            key={account._id || account.id}
                                            initial={{ opacity: 0 }}
                                            animate={{ opacity: 1 }}
                                            exit={{ opacity: 0 }}
                                            className="transition-colors hover:bg-white/35"
                                        >
                                            <td className="px-8 py-5">
                                                <div className="flex items-center gap-3">
                                                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-[#d7bc73]/45 bg-[#fff8df] text-[#8a611b] shadow-[0_12px_24px_rgba(154,107,22,0.12)]">
                                                        <CreditCard size={16} />
                                                    </div>

                                                    <span className="font-mono text-sm font-black text-[#3f2c12]">
                                                        {account.numeroCuenta}
                                                    </span>
                                                </div>
                                            </td>

                                            <td className="px-8 py-5">
                                                <span
                                                    className={`inline-flex rounded-full border px-3 py-1 text-xs font-black capitalize ${
                                                        account.tipoCuenta === 'ahorro'
                                                            ? 'border-[#d7bc73]/50 bg-[#fff8df] text-[#8a611b]'
                                                            : 'border-emerald-200 bg-emerald-50 text-emerald-700'
                                                    }`}
                                                >
                                                    {account.tipoCuenta}
                                                </span>
                                            </td>

                                            <td className="px-8 py-5 text-sm font-black text-[#3f2c12]">
                                                {fmt(account.saldo)}
                                            </td>

                                            {esAdmin && (
                                                <>
                                                    <td className="px-8 py-5 font-mono text-xs font-semibold text-[#7a6849]">
                                                        {getUsuarioTexto(account.usuario) || '—'}
                                                    </td>

                                                    <td className="px-8 py-5">
                                                        <span
                                                            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-black ${
                                                                account.estado
                                                                    ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
                                                                    : 'border-[#d7bc73]/40 bg-[#ead9ad]/35 text-[#8a6a3a]'
                                                            }`}
                                                        >
                                                            <span
                                                                className={`h-1.5 w-1.5 rounded-full ${
                                                                    account.estado
                                                                        ? 'bg-emerald-600'
                                                                        : 'bg-[#9a6b16]'
                                                                }`}
                                                            />
                                                            {account.estado ? 'Activa' : 'Inactiva'}
                                                        </span>
                                                    </td>

                                                    <td className="px-8 py-5">
                                                        <div className="flex items-center justify-end gap-2">
                                                            <button
                                                                type="button"
                                                                onClick={() => setDepositAccount(account)}
                                                                className="rounded-xl border border-emerald-200 bg-emerald-50/80 p-2 text-emerald-700 transition-all hover:bg-emerald-100"
                                                                title="Depositar a esta cuenta"
                                                            >
                                                                <Banknote size={16} />
                                                            </button>

                                                            <button
                                                                type="button"
                                                                onClick={() => {
                                                                    setEditingAccount(account)
                                                                    setShowForm(true)
                                                                }}
                                                                className="rounded-xl border border-[#d7bc73]/45 bg-white/50 p-2 text-[#8a611b] transition-all hover:bg-[#fff8df] hover:text-[#3f2c12]"
                                                                title="Editar cuenta"
                                                            >
                                                                <Edit3 size={16} />
                                                            </button>

                                                            <button
                                                                type="button"
                                                                onClick={() => setConfirmId(account._id || account.id)}
                                                                className="rounded-xl border border-red-200 bg-red-50/80 p-2 text-red-700 transition-all hover:bg-red-100"
                                                                title="Desactivar cuenta"
                                                            >
                                                                <Trash2 size={16} />
                                                            </button>
                                                        </div>
                                                    </td>
                                                </>
                                            )}
                                        </motion.tr>
                                    ))}
                                </AnimatePresence>
                            )}
                        </tbody>
                    </table>
                </div>
            </div>
        </motion.div>
    )
}

export default AccountList
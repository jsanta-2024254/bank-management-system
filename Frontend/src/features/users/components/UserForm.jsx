...
const UserForm = ({ user, onClose }) => {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
        defaultValues: {
            ...
            ingresosMensuales: user?.ingresosMensuales || user?.ClientProfile?.IngresosMensuales || '',
    ...
    const onSubmit = async (data) => {
        const toastId = toast.loading(estaEditando ? 'Actualizando usuario...' : 'Creando usuario...')
        ...
        try {
            ...
            toast.error(error?.response?.data?.message || `Error al ${estaEditando ? 'actualizar' : 'crear'} el usuario`, { id: toastId })
        }
    }
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input {...register('nombre', { required: 'El nombre es requerido' })} placeholder="Nombre" className={inputClass} />
                    ...
                    <div>
                        ...
                        <input {...register('apellido', { required: 'El apellido es requerido' })} placeholder="Apellido" className={inputClass} />
                ...
                {!estaEditando && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider mb-2 block">DPI</label>
                            <input
                                {...register('dpi', {
                                ...
                                className={inputClass}
                            />
                            {errors.dpi && <p className="text-red-400 text-xs mt-1.5 ml-1">{errors.dpi.message}</p>}
                        </div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider mb-2 block">Celular</label>
                        <input
                            {...register('celular', {
                                required: 'El celular es requerido',
                                pattern: { value: /^\d{8}$/, message: 'El celular debe tener 8 dígitos' },
                            })}
                            placeholder="55555555"
                            className={inputClass}
                        />
                        {errors.celular && <p className="text-red-400 text-xs mt-1.5 ml-1">{errors.celular.message}</p>}
                    </div>

                    <div>
                        <label className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider mb-2 block">Ingresos mensuales</label>
                        <input
                            {...register('ingresosMensuales', {
                                required: 'Los ingresos mensuales son requeridos',
                                min: { value: 100, message: 'Los ingresos deben ser al menos Q100' },
                            })}
                            type="number"
                            min="100"
                            step="0.01"
                            ...
                            className={inputClass}
                        />
                        {errors.ingresosMensuales && <p className="text-red-400 text-xs mt-1.5 ml-1">{errors.ingresosMensuales.message}</p>}
                    </div>
                </div>

                <div>
                    <label className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider mb-2 block">Dirección</label>
                    <input {...register('direccion', { required: 'La dirección es requerida' })} placeholder="Dirección" className={inputClass} />
                    {errors.direccion && <p className="text-red-400 text-xs mt-1.5 ml-1">{errors.direccion.message}</p>}
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider mb-2 block">Lugar de trabajo</label>
                        <input {...register('nombreTrabajo', { required: 'El lugar de trabajo es requerido' })} placeholder="Empresa / trabajo" className={inputClass} />
                        {errors.nombreTrabajo && <p className="text-red-400 text-xs mt-1.5 ml-1">{errors.nombreTrabajo.message}</p>}
                    </div>

                    {!estaEditando && (
                        <div>
                            <label className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider mb-2 block">Tipo de cuenta</label>
                            <select {...register('tipoCuenta')} className={`${inputClass} appearance-none cursor-pointer`}>
                ...
                <div className="flex gap-4 pt-4">
                    ...
                    >
                        {isSubmitting ? (estaEditando ? 'Guardando...' : 'Creando...') : (estaEditando ? 'Guardar Cambios' : 'Crear Usuario')}
                    </button>
                </div>
            </form>
        </Modal>
